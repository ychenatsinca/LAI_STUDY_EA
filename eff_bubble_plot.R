...
load("./Rda_4050_Obs6/WP_20010110to20181231_table.combw_3_p_350_TY_125D.rda")
...
mon.txt <- c("Jan","Feb","Mar","Apr","May","Jun","Jul","Agu","Sep","Otc","Nov","Dec")
...
x_pos <- (as.numeric(table.comb$date.mon)+as.numeric(table.comb$date.day)/30)-0.5
...
par(mar = c(4, 4, 0, 0))
   plot(x=x_pos , y=y_pos, xlab="Month", ylab="Latitude", cex.lab=1.5, cex.axis=1.5,
        col=out_col, bg=table.comb$col, xlim=c(1,13), ylim=c(8,41.5), cex=eff_cex, pch=21,lwd=1.5)
   grid()
#plot 4 seasonal average of the effect size
par(mar = c(0, 4, 3, 0))
   ...
   plot(y=table.comb$eff.size.for, x=(table.comb$date.mon+ as.integer(table.comb$date.day)/30 ),
         col="white",ylim=c(-0.3,0.4),xlim=c(1,13),
         xlab="", ylab="Effect size",main="", xaxt = "n" )
 
   #get file-list 
   in.files <- list.files(path="./Rda_4050_Obs6/", pattern="WP_20010110to20181231_table.combw_*")
   for (id in 1:length(in.files)) { 
   #for (id in 1:1) { 
   load(paste("./Rda_4050_Obs6/",in.files[id],sep=""))
   table.comb$date.day <- substr(table.comb$date.time,start=7,stop=8)
   #subset datataset 
...
par(mar = c(4, 0, 0, 0))
   ...
   plot(x=table.comb$eff.size.for, y=table.comb$eve.lat.med.aff,ylim=c(8,41.5), 
        col="white",xlim=c(-0.3,0.3) ,ylab="", xlab="Effect size",main="", yaxt = "n" )
   ...
   load("./Rda_4050_Obs6/WP_20010110to20181231_table.combw_5_p_350_TS_125D.rda")
   table.comb$date.day <- substr(table.comb$date.time,start=7,stop=8)
   #subset datataset
   table.comb <- subset(table.comb, table.comb$eve.for.pix.aff >= 5000)
...
par(mar = c(0.5, 5, 3.5, 2), xaxt = "n")
    color.bar(lut=eff.col,min=-0.8,title="Effect size scale")
#close devic()
if(go.pdf){dev.off()}
ld.go <- FALSE
if (ld.go) {
