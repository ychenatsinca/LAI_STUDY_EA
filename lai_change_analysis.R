...
#          qc2: for checking mean difference caused by the TC event    
...
#}
  ...
  # add the check of the mean difference of LAI before the TC event in the reference and the affect area 
   ...
   aa2 <- mean(lai_af*eve.tc.ref.mask*lc.for.mask,na.rm=T) - mean(lai_be*eve.tc.ref.mask*lc.for.mask,na.rm=T)
     ...
     print(paste("Forst LAI after in reference area:",  aft.for.lai.ref) )
  ...
  # we will only analysis the event with mean difference over than 0.25 = sqrt(0.185^2 + 0.185^2)  
   if ( is.na(aa) != TRUE ) {
        if( ( abs(aa) <= 0.2) | (is.na(eff.size.for) == TRUE) ) {
        # exit the TC event loop  
          print( "QC1 TC event, the normalized difference smaller than 0.2!") 
          qc2.flag <- FALSE 
       }
...
if (ld_go) {
  ...
  if ( is.na(eff.size.for) != TRUE ) {
     table.tmp <- data.frame( date.time = wrk.date,
                              ...
                              #eff.size.acf.for = eff.size.acf.for,
                              qc1.score = qc1.score,
                              qc2.score = qc2.score,
                              qc1.flag = qc1.flag,
                              qc2.flag = qc2.flag  )
...
#  home.dir <- c("/data1/home/ychen/")
  ...
  save(table.comb, file = paste(target,"_",st.date,"to",ed.date,"_table.comb",
       "w_",Ref.wind,"_p_",Ref.rain ,"_",pos.txt,"_",off.txt,"_cut_",cut.off,".rda",sep="") )
